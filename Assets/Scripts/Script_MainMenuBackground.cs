...
public class Script_MainMenuBackground : MonoBehaviour
{
    int topLeftX = -50; 
    int topLeftY = 1130; 
    ...
    private void Update()
    {
        ...
        if (((firstTile.transform.position.y > 0 && (int)firstTile.transform.position.y % 100 == 30) ||
             (firstTile.transform.position.y < 0 && Mathf.Abs((int)firstTile.transform.position.y) % 100 == 70)) &&
        ...
        if ((firstTile.transform.position.y > 0 && (int)firstTile.transform.position.y % 100 == 80) ||
            (firstTile.transform.position.y < 0 && Mathf.Abs((int)firstTile.transform.position.y) % 100 == 20))
        {
            EN = true;
        }
    }
